...
"session_start" message, encrypted with the newly established session
...
6.  If the session is accepted, send an acknowledgement message
    ...
    - The special message "session_start" in ASCII (13 bytes)
...
Once the client receives the "session_start" message, it also accepts
...
2.  Client verifies message content
    ...
    - The client verifies that the Id sent is its own ID, that the
      message is "session_start", and that the timestamp is within 1
      minute ago.
...
client has not received the session_start message yet, the client drops
...
Used to send the session_start message. AES in GCM mode is used, with
...
diagram](Handshake_State_Diagram_Server.png)
...
diagram](Handshake_State_Diagram_Client.png)
...
it receives and validates the session_start message. The FAST Tunnel is
also responsible for the session_end message, as illustrated in the
state diagram above.
...
![Tunnel protocol message sequence diagram](Tunnel_MSC.png)
...
diagram](Tunnel_State_Diagram_Server.png)
...
diagram](Tunnel_State_Diagram_Client.png)
...
In the second step, the authenticity and integrity of the start_session
